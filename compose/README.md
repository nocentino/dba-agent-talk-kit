...
### Demo notes
- `sqlserver3`: `MSSQL_AGENT_ENABLED=true` — required for the SQL Agent job-failure
  demo (`demos/bonus-maintenance-neglect.md`). Seed with
  `demos/sql/seed-maintenance-neglect.sh`, reset with the matching reset script.
...
```
(`--profile fleet` ensures the fleet containers/volumes are included even if
they were only ever started once.)
